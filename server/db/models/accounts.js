...
tosAccountSchema.statics.getToken = code =>
...
tosAccountSchema.statics.createOrRefreshToken = accountName =>
  new Promise((resolve, reject) => {
    const headers = {
      'Content-Type': 'application/x-www-form-urlencoded'
    };
...
tosAccountSchema.statics.getRefreshToken = ({ accountName }) =>
  new Promise((resolve, reject) => {
    TosAccounts.findOne({ accountName })
      .exec()
      .then(dbAccount => {
        ...
        request(options, (error, response, body) => {
          ...
          reject(
            new Error(JSON.stringify({ status: response.statusCode, body }))
          );
        });
      })
      .catch(reject);
